		...
		return (
			<>
				...
				<div style={{"width": "100%", "display": "flex"}}>
					...
					<div style={{"width": "calc(100% / 3)"}}>
						<Titulo entidad="Venta" emoji="🛍️" />
						<MusicoList musicos={this.state.musicos} />
						<Link to="/nuevo-musico">Nueva Venta</Link>
					</div>
					<div style={{"width": "calc(100% / 3)"}}>
						<Titulo entidad="VentaDetalle" emoji="🧾" />
						<BandaList bandas={this.state.bandas} />
						<Link to="/nueva-banda">Nueva VentaDetalle</Link>
					</div>
				</div>
	...
	render() {
		...
		return (
			<tr>
				...
				<td>
					<Link to={`/ver-banda/${id}`}>Ver VentaDetalle</Link>
				</td>
			</tr>
		)
	}
